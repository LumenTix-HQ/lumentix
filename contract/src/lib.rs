...
impl LumentixContract {
    ...
    pub fn refund_ticket(env: Env, ticket_id: u64, buyer: Address) -> Result<(), LumentixError> {
        ...
        // Update event: decrement tickets_sold to free up capacity
    ...
    /// Get remaining ticket availability for an event
    /// Returns the number of tickets still available (max_tickets - tickets_sold)
    pub fn get_availability(env: Env, event_id: u64) -> Result<u32, LumentixError> {
        ...
        // Calculate remaining tickets, ensuring non-negative result
        let availability = event.max_tickets.saturating_sub(event.tickets_sold);
